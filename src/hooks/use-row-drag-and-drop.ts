...
import { findRowUnderPoint, resolveDropAction, type DropAction, type DropTarget, type Rect } from '@/lib/drag-drop-groups';
import type { PaymentRow } from '@/lib/payments-dashboard';

// Stage 18 follow-up (drag-and-drop grouping) — owns the gesture/geometry
// side of the Dashboard's drag-to-group interaction: a bounds registry
// (every draggable row and group header registers its measured on-screen
// rect via onLayout/measureInWindow), the currently-dragged row's visual
// offset (shared values, read by each row's own useAnimatedStyle), and
// which target is currently under the pointer (plain state — the
// highlight doesn't need worklet-level smoothness). The *domain* decision
// (what a drop actually means) is src/lib/drag-drop-groups.ts's pure
...
// onDropResolved's caller — this hook never calls a store function
// itself.
//
...
export function useRowDragAndDrop(onDropResolved: (draggedRow: PaymentRow, action: DropAction) => void) {
  const boundsRef = useRef(new Map<string, Rect>());
  const targetsRef = useRef(new Map<string, DropTarget>());
  const draggedRowRef = useRef<PaymentRow | null>(null);
  ...
  function registerTarget(id: string, rect: Rect, target: DropTarget) {
  ...
  function handleDragStart(row: PaymentRow) {
  ...
  function handleDragEnd() {
    ...
    if (draggedRow) {
      ...
      const target: DropTarget | null = hitTarget ?? (draggedRow.recurringGroupId ? { kind: 'outsideGroup' } : null);
...
export type RowDragAndDrop = ReturnType<typeof useRowDragAndDrop>;
