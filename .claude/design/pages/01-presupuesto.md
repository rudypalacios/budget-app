...
4. ~~El aspecto de la banda (chip warning + dos botones) **no cambia**~~ *(superado por D14: ya tiene el aspecto del v9)*; los textos de los dos
...
- **D12:** estimado estándar reutilizable (IQR + promedio) en un módulo compartido, y usarlo también en el promedio de 6 meses de las recomendaciones de recurrentes.
...
| D9 | Detalle: ver qué compone las cifras | **Se añade** (fase 5) la sección "Movimientos del mes" antes de "Recurrentes": todos los gastos del ciclo de la categoría (pendientes por fecha de vencimiento, luego pagados por fecha de pago), únicos y recurrentes, excluyendo omitidos, archivados y en papelera — los mismos filtros de `pending`/`actual`, así que la lista siempre suma lo que muestra la tarjeta (`categoryMovements`). Solo lectura; chip Pendiente (warning) / Pagado (neutro), sin marcar vencidos (eso es del Panel). Primeros 10 + "Ver todos (N)". |
| D10 | Sugerido de "Fijar presupuesto" (y de editar categoría) | **Cambia** (revisión de la fase 6): ya no es la suma de las recurrentes (`suggestCategoryMonthlyBudget`, retirada). Es el **promedio mensual de todo lo pagado en la categoría** — únicos y recurrentes — en los últimos 6 meses completos, contando solo desde el mes del primer gasto (`averageMonthlySpending`). Sin un mes completo de historia no hay sugerido. Motivo: el presupuesto de una categoría cubre todo lo que se registra en ella, sea recurrente o no. | **Actualizado:** los meses atípicos se excluyen con la regla de Tukey (fuera de Q1 − 1,5·IQR … Q3 + 1,5·IQR, cuartiles como medianas de cada mitad) antes de promediar los totales mensuales; con menos de 4 meses no se filtra. La mediana sola se descartó porque da 0 en categorías esporádicas. En la hoja se muestra solo "Presupuesto sugerido: X" con un ⓘ que despliega (en línea, sin otra hoja) una explicación breve sin tecnicismos; `suggestCategoryBudget` reemplaza a `averageMonthlySpending`.
| D11 | Detalle: "Recurrentes" duplicaba "Movimientos del mes" | **Se retira la sección "Recurrentes"** (§5.6 punto 3; revisión de la fase 6). Las recurrentes aparecen solo como sus movimientos del mes, con un ícono de recurrencia (↻, `SymbolView` `arrow.triangle.2.circlepath` / `autorenew`, etiqueta accesible "Recurrente") junto al nombre — no chip, para no saturar la fila; el Panel adoptará el mismo ícono en su propio rediseño. Si su definición tiene recomendación pendiente, el aviso (`BudgetRecommendationBadge`) va debajo de su primer movimiento visible. Se retiran la línea de promedio, "edítalo en Gastos" y "sin recurrentes" (claves `budget.detail.recurring/noRecurring/editInExpenses`, `budget.recurringAverage`, `budget.noHistoryYet`). Una recurrente sin movimiento este mes no se muestra aquí; su recomendación sigue en Gastos. |
| D12 | Estimado estándar con exclusión de atípicos en toda la app | **Hecho en la fase 8** (`src/lib/estimate.ts`: `withoutOutliers` + `robustAverage`, usado por `suggestCategoryBudget` y `computeBudgetRecommendation`). Plan original: La regla IQR + promedio (`withoutOutliers` / `suggestCategoryBudget`) se vuelve la función **estándar y reutilizable** para cualquier estimado de la app: mover la estadística a un módulo compartido (p. ej. `src/lib/estimate.ts`: totales → quitar atípicos → promedio) y hacer que el promedio de 6 meses de las recomendaciones de recurrentes (`computeBudgetRecommendation`, etapa 13 / FR-6a) lo use en lugar de su promedio simple. Una sola implementación, sin duplicar. Cambia cuándo aparecen avisos: se prueba aparte. |
| D13 | Recomendación de la recurrente vs. de la categoría | **Recurrente:** se mantiene el aviso activo (Actualizar / Mantener) bajo su movimiento — corrige un *pronóstico* (el monto esperado alimenta "Falta pagar" y la proyección). **Categoría:** sin aviso insistente — su presupuesto es una *meta*, no un pronóstico; el chip "Sobre presupuesto" ya avisa, y un aviso de categoría duplicaría en cascada el de la recurrente que lo causa. En su lugar, en el detalle de una categoría con presupuesto: el botón **"Ajustar presupuesto"** (el "Presupuesto sugerido" con su ⓘ se muestra solo dentro de la hoja, no también en la tarjeta, para no duplicarlo), que abre la misma hoja de "Fijar presupuesto" prellenada con el valor actual (toast "…actualizado a…"). Esto reemplaza la regla de §1 de que Presupuesto no edita metas existentes; **quitar** un presupuesto sigue solo en Ajustes. Sin cambios de modelo de datos. Posible extensión futura, no ahora: avisar si la categoría se excede 3 meses seguidos. |
| D14 | Ajuste visual al v9 del detalle | **Aplicado en la revisión de la fase 6** (no en la fase 7): el aviso de recomendación (`BudgetRecommendationBadge`, compartido con Gastos) pasa de chip + enlaces a **recuadro ámbar con los dos botones dentro, del mismo ancho** (como `.chipw`/`.chb` del v9); divisoria fina entre la cara de la tarjeta y su detalle; separadores entre filas de movimientos; **borde rojo** en tarjetas `over`; chips compactos (`Chip size="small"`) en las filas. `Button` centra su texto cuando se parte en dos líneas. Reemplaza la línea de la fase 7 que decía que la banda "no cambia" de aspecto. |
| D15 | Duración del toast con acciones (fase 7) | **Un toast con acciones no se cierra solo:** permanece hasta que el usuario elige una acción (Deshacer / Editar) o toca la ✕. Los toasts informativos (sin acciones) siguen cerrándose a los 2,5 s. Reemplaza el `durationMs`/8000 ms de la fase 7. Además, Actualizar/Mantener no esperan la confirmación del servidor (la escritura queda en caché local y se sincroniza después, igual que "Fijar presupuesto"); un rechazo real muestra un toast de error. |
| D16 | Re-skin tras el cierre | **Lo global se vuelve global:** la paleta, la tipografía (fuente del sistema, pesos 400/500, título de página de 22 px) y el estilo de tarjeta (borde fino, radio 12) del v9 pasan a ser el tema de toda la app; las demás páginas solo heredan los tokens, sin cambios propios (cada una tendrá su ciclo). En esta página: cifras del resumen con signo antes de la moneda (`+Q` entra, `−Q` sale), subtítulo con el mes, chips pequeños con ícono, barra de 6 px y chevron abajo/arriba. **Se mantiene D6** (sin rótulo "Lo que falta"). Reemplaza, para lo global, la regla 0.11 de no tocar constantes de estilo. |
