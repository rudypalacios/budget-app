...
| D11 | Detalle: "Recurrentes" duplicaba "Movimientos del mes" | **Se retira la sección "Recurrentes"** (§5.6 punto 3; revisión de la fase 6). Las recurrentes aparecen solo como sus movimientos del mes, con chip "Recurrente". Si su definición tiene recomendación pendiente, el aviso (`BudgetRecommendationBadge`) va debajo de su primer movimiento visible. Se retiran la línea de promedio, "edítalo en Gastos" y "sin recurrentes" (claves `budget.detail.recurring/noRecurring/editInExpenses`, `budget.recurringAverage`, `budget.noHistoryYet`). Una recurrente sin movimiento este mes no se muestra aquí; su recomendación sigue en Gastos. |
